...
## 6.1 توافق الواجهة القديمة (نافذة النشر)

النشر Backend-أولاً يجب ألا يكسر الواجهة المنشورة حاليًا:
- **RPC الجديدة `decide_leave_request`**: سبب الرفض **إلزامي** (بعد `trim` غير فارغ، حد أقصى 1000 محرف، يُرفض الفارغ/المتجاوز برسالة واضحة بلا قصّ صامت).
- **حارس الـTrigger `fn_leave_decision_guard`**: حارس أمان **عامّ** ومتوافق مع الكتابة القديمة — تحوّل مباشر (UPDATE) من معلّق إلى مرفوض **بلا `reject_reason`** لا يفشل؛ يُخزَّن `reject_reason = NULL` (لا يُخترع سبب منسوب للمسؤول). يبقى: المنفّذ من `auth.uid()`، الدور/الوقت من الخادم، صلاحية النطاق (`can_write_team`)، القرار مرّة واحدة، والتسجيل/الإشعار/التدقيق.
- **العرض عند غياب السبب**: نص الإشعار/الواجهة يعرض «لم يُذكر سبب الرفض»؛ ويبقى `leave_decisions.reason = NULL` (لا تُخزَّن عبارة العرض كأنها سبب المسؤول). `data` لا يكرّر السبب.
- **لا آلية قابلة للتزوير**: لا معامل `legacy/skip_validation` ولا اعتماد على JWT؛ التمييز طبيعي — الـRPC تتحقّق قبل الـUPDATE، والحارس يبقى عامًّا.
- **لاحقًا** (بعد انتهاء توافق العملاء ذوي الكاش القديم): Migration منفصلة تُزيل UPDATE المباشر للحالة وتشدّد إلزام السبب على جميع المسارات.

## 6.2 حدود النصوص (منع تضخيم القاعدة)

- `decide_leave_request.p_reason` ≤ 1000 · `submit_leave_request.p_notes` ≤ 2000 — يُرفض المتجاوز (لا قصّ صامت).
- قيود جدول: `leaves.reject_reason` ≤ 1000 (يسمح NULL) · `leave_decisions.reason` ≤ 1000 (NULL-able) · `notifications.title` ≤ 200 · `body` ≤ 2000 · `data` ≤ 16KB (نص). `data` يُنشأ داخليًا فقط (لا من العميل) ولا يكرّر سبب الرفض.
