...
BEGIN
  ...
  WITH src AS (
    SELECT (e->>'id')::uuid                        AS id,
           btrim(coalesce(e->>'variant_name', ''))    AS variant_name,
           btrim(coalesce(e->>'variant_name_en', '')) AS variant_name_en,
           btrim(coalesce(e->>'sku', ''))             AS sku,
           btrim(coalesce(e->>'barcode', ''))         AS barcode,
           btrim(coalesce(e->>'color', ''))           AS color,
           btrim(coalesce(e->>'size', ''))            AS size,
  ...
  WITH src AS (
    SELECT btrim(coalesce(e->>'variant_name', ''))    AS variant_name,
           btrim(coalesce(e->>'variant_name_en', '')) AS variant_name_en,
           btrim(coalesce(e->>'sku', ''))             AS sku,
           btrim(coalesce(e->>'barcode', ''))         AS barcode,
           btrim(coalesce(e->>'color', ''))           AS color,
...
--   select public.sync_product_variants('<product-uuid>'::uuid, '[]'::jsonb);
--   -- expect {"ok": true, "updated": 0, "inserted": 0, "deleted": N}
--   -- on a product whose variants are all removable.
