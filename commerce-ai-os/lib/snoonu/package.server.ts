// INT.2C — Snoonu outbound package GENERATOR (SERVER-ONLY).
//
// Turns ONE Snoonu storefront's certified preview into the downloadable package:
//   <Store>/snoonu-products.xlsx  +  <Store>/images/<SKU>.<ext>  +  manifest.json
//
// It CONSUMES loadSnoonuPreview(storefrontKey) — the single bounded read +
// storefront-scoped ECL identity + validation — and never re-derives identity,
// never reads the legacy per-store id columns, and never lets the two
// storefronts share state (each generation is scoped to one storefront_key).
// Only NOT-blocked rows are packaged; blocked rows are always excluded. It
// mutates NO business data (no catalog/inventory/availability/ECL writes, no
// Snoonu API publish): the sole write is a best-effort malak_audit trail row.
// Images are fetched through the certified SSRF-safe boundary and verified before
// packaging. Lives OUTSIDE lib/export (which is read-only + generation-free).
...
import { buildSnoonuXlsxBuffer } from "@/lib/snoonu/package-xlsx";
import { loadSnoonuPreview } from "@/lib/export/snoonu/preview.server";
import type { SnoonuStorefrontKey, SnoonuPreviewRow } from "@/lib/export/snoonu/preview";
import {
  ...
  primaryFilenameFor,
  PACKAGE_LIMITS,
  type SnoonuGenerationMode,
  type SnoonuPackageRow,
  type PackagedFile,
} from "@/lib/export/snoonu/package";
import { sniffImageExtension, mimeToExt } from "@/lib/export/package-core";

const IMAGE_FETCH_CONCURRENCY = 6;

const STORE_FOLDER: Record<SnoonuStorefrontKey, string> = {
  "snoonu:malikas": "Snoonu-Malikas",
  "snoonu:pure_seoul": "Snoonu-PureSeoul",
};
const STORE_SLUG: Record<SnoonuStorefrontKey, string> = {
  "snoonu:malikas": "malikas",
  "snoonu:pure_seoul": "pure-seoul",
};
...
export interface SnoonuPackageSummary {
  storefrontKey: SnoonuStorefrontKey;
  mode: SnoonuGenerationMode;
  generatedAt: string;
  actor: string | null;
...
export type GeneratePackageResult =
  | { ok: true; filename: string; bytes: Uint8Array; summary: SnoonuPackageSummary }
  | { ok: false; error: GeneratePackageError };

export type GeneratePackageError = "preview_unavailable" | "no_exportable_rows" | "integrity_failed" | "generation_failed";

interface ResolvedImage { bytes: Uint8Array; filename: string; kind: "primary" | "gallery"; ownerPrimary?: string }
interface ResolvedRow { row: SnoonuPreviewRow; primary: ResolvedImage; gallery: ResolvedImage[] }
...
/**
 * Generate the Snoonu package for ONE storefront. Caller MUST have enforced the
 * writer authorization boundary. Read-only against the catalog; the only write is
 * a best-effort audit row.
 */
export async function generateSnoonuPackage(opts: GeneratePackageOptions): Promise<GeneratePackageResult> {
  const now = opts.now ?? new Date();
  const startedAt = now.toISOString();
  const storefrontKey = opts.storefrontKey;

  const preview = await loadSnoonuPreview(storefrontKey);
  if (!preview) return { ok: false, error: "preview_unavailable" };

  const set = resolveSnoonuGenerationSet(preview.rows, { mode: opts.mode, selectedKeys: opts.selectedKeys });
  ...
  try {
    const resolvedRows = await mapPool(capped, IMAGE_FETCH_CONCURRENCY, async (row): Promise<ResolvedRow | null> => {
      ...
      if (!plan.primary) return null; // no retrievable primary → excluded
    ...
    const packageRows: SnoonuPackageRow[] = survivors.map((s) => toPackageRow(s.row, s.primary.filename));
    const rowImageRefs = packageRows.map((r) => r.imageFilename);
    ...
    const xlsxBytes = buildSnoonuXlsxBuffer(packageRows);

    const mappedCount = survivors.filter((s) => s.row.spi !== null).length;
    ...
    const folder = STORE_FOLDER[storefrontKey];
    const outputFilename = `snoonu-${STORE_SLUG[storefrontKey]}-export-${fileStamp(now)}.zip`;
    ...
    const entries: ZipEntry[] = [{ name: `${folder}/snoonu-products.xlsx`, data: xlsxBytes }];
    for (const s of survivors) {
    ...
    const summary: SnoonuPackageSummary = {
      storefrontKey, mode: opts.mode, generatedAt: startedAt, actor: opts.actor, outputFilename,
      productRowCount: survivors.length, mappedCount, unmappedCount, imageCount, warningCount,
      excludedBlockedCount: set.excludedBlocked.length, excludedNoImageCount, cappedExcludedCount, integrityOk: true,
    };
    ...
    await recordAudit(
      {
        storefrontKey, mode: opts.mode, generatedAt: startedAt, actor: opts.actor, outputFilename: "",
        productRowCount: 0, mappedCount: 0, unmappedCount: 0, imageCount: 0, warningCount: 0,
        excludedBlockedCount: set.excludedBlocked.length, excludedNoImageCount: 0, cappedExcludedCount, integrityOk: false,
      },
      "error",
      new Date(),
    ).catch(() => {});
...
/**
 * Durable Export History via the EXISTING audit framework (malak_audit) — no new
 * table, no migration. Best-effort: a failed audit never fails the package.
 */
async function recordAudit(summary: SnoonuPackageSummary, status: "done" | "error", finishedAt: Date): Promise<void> {
  try {
    const admin = createAdminClient() as never;
    await insertAuditRow(admin, {
      action_type: "snoonu_package_export",
      ...
      details: {
        destination: summary.storefrontKey,
        actor: summary.actor,
        mode: summary.mode,
