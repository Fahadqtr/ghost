...
function defaultOnly(): RepairLiveProduct {
  return { id: GID, variants: [{ id: "gid://shopify/ProductVariant/1", sku: "mk1597", barcode: "", title: DEFAULT_TITLE }] };
}
function liveWith(variants: RepairLiveVariant[]): RepairLiveProduct {
  return { id: GID, variants };
}

// ── planning ─────────────────────────────────────────────────────────────────

test("plan: default-only live + complete internal variants → READY with exact creates", () => {
