// RAFEEQ TEMPLATE BARCODE RULE (owner decision) on the NATIVE template — the
// Rafeeq barcode cell carries the canonical PARENT product SKU on every
// physical row:
//   1. simple product  → barcode = the product's own (parent) SKU
//   2. product with options → EVERY repeated option row carries the SAME
//      parent SKU in the barcode cell (confirmed live in the audited workbook)
...
function variant(id: string, sku: string, eanCode: string, over: Partial<RafeeqPreviewVariant> = {}): RafeeqPreviewVariant {
  return { id, sku, barcode: eanCode, nameEn: `Option ${id}`, nameAr: `خيار ${id}`, price: null, ...over };
}
...
function exportedCells(products: RafeeqPreviewProduct[]): { barcodes: string[]; all: string[] } {
  ...
  return { barcodes: data.map((r) => String(r[NATIVE_COL.barcode])), all: data.flatMap((r) => r.map((c) => String(c))) };
}

// ── 1) simple product: Rafeeq barcode = parent SKU ────────────────────────────
test("1: a simple product exports its own (parent) SKU in the Rafeeq barcode cell", () => {
...
test("2: every repeated option row carries the SAME parent SKU in the barcode cell", () => {
...
test("3: the real EAN/product barcode never reaches the Rafeeq barcode cell", () => {
  ...
  for (const b of barcodes) {
    ...
    for (const e of VARIANT_EANS) assert.notEqual(b, e, "variant EAN never exported as barcode");
  }
  assert.deepEqual(barcodes, ["mk175", "mk200"]);
});

// ── 4) a variant SKU is never written to the Rafeeq barcode ───────────────────
test("4: a variant SKU never reaches the Rafeeq barcode cell", () => {
  const p = product("p1", "mk175", {
  ...
  const { barcodes } = exportedCells([p]);
  for (const b of barcodes) {
    assert.ok(!variantSkus.has(b), "barcode is never the variant's own SKU");
    assert.equal(b, "mk175");
  }
});
...
test("5: variant barcodes never appear ANYWHERE in the exported spreadsheet or image plan", () => {
  ...
  for (const cell of all) {
    for (const e of VARIANT_EANS) assert.ok(!cell.includes(e), `variant barcode ${e} leaked into cell "${cell}"`);
  }
  // the image plan (packaged filenames) is parent-SKU based — no EAN there either
  ...
  for (const r of rows) {
    ...
    for (const n of names) for (const e of VARIANT_EANS) assert.ok(!n.includes(e));
  }
});
...
test("the same parent SKU claimed by TWO different products blocks both (corrupted identity/grouping key)", () => {
