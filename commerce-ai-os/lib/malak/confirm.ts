...
function secret(): string {
  // Server-only; never exposed to the client. Present whenever the admin
  // client works (same key powers the write itself).
  const s = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.MALAK_SIGNING_SECRET;
  if (!s) throw new Error("Missing signing secret (SUPABASE_SERVICE_ROLE_KEY).");
  return s;
}
