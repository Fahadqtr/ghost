...
export default function MalakHud() {
  ...
  return (
    <div dir="ltr" className="relative -m-4 min-h-[calc(100vh-2rem)] overflow-x-hidden font-mono sm:-m-6" style={{ background: "#020510", color: CY }}>
      <style>{`
      ...
      {/* HEADER */}
      <div className="relative z-10 flex flex-wrap items-start justify-between gap-3 px-3 pt-4 sm:px-5 sm:pt-5">
        <div>
        ...
        <div className="text-right">
          <p className="text-[20px] font-bold leading-none tracking-wider sm:text-[26px]" style={{ color: "#cfeeff", textShadow: `0 0 12px ${CY}88` }}>{now}</p>
      ...
      {/* MAIN GRID — stacks on mobile (orb first), 12-col on lg+ */}
      <div className="relative z-10 grid grid-cols-1 gap-3 px-3 pt-4 sm:px-5 lg:grid-cols-12">
        {/* LEFT */}
          ...
          <Panel title="ACTION QUEUE" right={<span className="text-[8.5px]" style={{ color: CY_DIM }}>{scan?.issues?.length ?? 0}</span>}>
            {scan?.allClear ? (
              ...
              <div className="space-y-1.5">
                {(scan?.issues ?? []).map((is) => (
                  <a key={is.key} href="/malak/chat" className="flex items-center justify-between gap-2 rounded border px-2 py-1.5 transition hover:bg-white/5"
        ...
        {/* CENTER — tap the orb to open the chat/voice screen */}
        <div className="order-1 flex flex-col items-center justify-start pt-2 lg:order-2 lg:col-span-6">
          <a href="/malak/chat" aria-label="افتح محادثة ملاك" className="flex flex-col items-center">
            <JarvisOrb state={state} size={orbSize} levelRef={levelRef} />
            <p className="mt-1 text-[10px] tracking-[0.35em]" style={{ color: CY_DIM }}>
              {loading ? "SCANNING" : "TAP TO TALK · اضغط للمحادثة"}
            </p>
          </a>
        </div>

        {/* RIGHT */}
        <div className="order-3 space-y-3 lg:col-span-3">
      ...
      {/* PRIMARY OBJECTIVE */}
      <div className="relative z-10 mx-3 mb-4 mt-4 rounded border p-3 sm:mx-5" style={{ borderColor: CY_FAINT, background: "rgba(8,20,40,0.4)" }}>
        ...
        <div className="mt-2 flex items-end gap-6">
          ...
          <a href="/malak/chat" className="shrink-0 rounded border px-4 py-2 text-[10px] tracking-widest transition hover:bg-white/5"
