...
export default function Office3D({
  ...
  useEffect(() => {
    ...
    const buildRobot = (a: OfficeAgent, station: number) => {
      ...
      const emojiSprite = new THREE.Sprite(new THREE.SpriteMaterial({ map: makeEmojiTexture(EMOJI_BY_ID[a.id] || "💡"), transparent: true, opacity: 0 })); emojiSprite.scale.set(0.5, 0.5, 0.5); emojiSprite.position.y = 2.15; group.add(emojiSprite);
      scene.add(group);
      robots[a.id] = {
        group, headPivot, leftArm, rightArm, eyeL, eyeR, ring, glowDisc, body, legL, legR,
        phase: Math.random() * Math.PI * 2, blinkT: 2 + Math.random() * 3, emojiSprite, emojiTarget: 0,
        home: { x: pos.x, z: pos.z }, beh: "work", behTimer: 2 + Math.random() * 3,
        target: null, heading: Math.atan2(-pos.x, -pos.z), walkPhase: 0, workPhase: Math.random() * Math.PI * 2, hop: 0,
      };
    };
    ...
    const moveToward = (r: any, dt: number) => {
      ...
      if (dlen < 0.18) return true;
      const sp = 1.5 * dt; g.position.x += (dx / dlen) * sp; g.position.z += (dz / dlen) * sp;
      r.heading = lerpAngle(r.heading, Math.atan2(dx, dz), 0.16); r.walkPhase += dt * 10; return false;
    };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const animate = () => {
      ...
      agents.forEach((a) => {
        ...
        r.emojiSprite.position.y = 2.15 + Math.sin(t * 2 + r.phase) * 0.06;
