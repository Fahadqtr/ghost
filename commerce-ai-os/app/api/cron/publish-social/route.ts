...
export async function GET(req: Request) {
  ...
  const { data: due, error } = await admin
    .from("social_posts")
    .select("id, platform, caption, image_url, extras")
    .eq("status", "pending")
    .eq("approved", true)
  ...
  for (const row of due as { id: string; platform: string; caption: string; image_url: string; extras: { kind?: string; storyKind?: string } | null }[]) {
    if (row.platform !== "instagram") { results.push({ id: row.id, skipped: row.platform }); continue; }
    const kind = row.extras?.kind ?? "post";
    const storyIsVideo = row.extras?.storyKind === "video" || isVid(row.image_url);

    // Publish according to the post kind: a scheduled Story posts only as a
    // story, a Reel as a reel, and a normal post to the feed (+ story mirror).
    const res = kind === "story"
      ? (storyIsVideo ? await publishVideoStoryToInstagram(row.image_url) : await publishStoryToInstagram(row.image_url))
      : kind === "reel"
        ? await publishReelToInstagram(row.image_url, row.caption)
        : await publishToInstagram(row.image_url, row.caption);
    ...
    if (res.ok) {
      ...
      let story = false;
      if (kind === "post") {
        // Mirror a normal feed post to the STORY too (best-effort).
        try { story = (await publishStoryToInstagram(row.image_url)).ok; } catch { story = false; }
      }
      results.push({ id: row.id, ok: true, kind, story });
    } else {
